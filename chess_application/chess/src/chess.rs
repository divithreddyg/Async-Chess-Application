...
impl Chess {
    ...
    pub fn remote_move(&mut self, source: (isize,isize), destination: (isize,isize)) -> Result<String, Error> {
            ...
            match self.move_piece_temp(source, destination) {
                Ok(()) => {
                    if self.is_under_checkmate(self.players[self.current_turn].get_color().other()) {
                        ...
                        return Err(Error::Checkmate("Checkmate".to_string()));
                    }
        ...
        Ok(self.chessboard.get_encoded_string())
    }
    ...
    /// Driver code for server-client (1v1)
    pub fn start_server(&mut self, server: &mut Communicator) {
        loop {
            self.chessboard.pretty_print();
            print!("Your turn: Enter source: ");
            let source = Self::get_move();
            print!("Your turn: Enter destination: ");
            let destination = Self::get_move();
            server.send(format!("{}-{}", source, destination)).unwrap();
            let source = Self::extract_position(&source);
            let destination = Self::extract_position(&destination);

            self.move_piece_temp(source, destination);
            self.chessboard.pretty_print();
            let move_ = server.recv().unwrap();
            println!("Opponent played: {}", move_);
            let mut parsed = move_.split(" ");
            let r = Self::extract_position(parsed.next().unwrap());
            let c = Self::extract_position(parsed.next().unwrap());
            self.move_piece_temp(r, c);
            self.chessboard.pretty_print();
            self.current_turn = (self.current_turn + 1) % 2;
        }
    }
    ...
    /// Driver code for server-client (1v1)
    pub fn start_client(&mut self, client: &mut Communicator) {
        loop {
            self.chessboard.pretty_print();
            let move_ = client.recv().unwrap();
            println!("Opponent played: {}", move_);
            let mut splitter = move_.split("-");
            let source  = Self::extract_position(splitter.next().unwrap());
            let destination  = Self::extract_position(splitter.next().unwrap());
            ...
            let source = Self::get_move();
            let destination = Self::get_move();
            client.send(format!("{}-{}", source, destination)).unwrap();
            self.current_turn = (self.current_turn + 1) % 2;
        }
    }
}
